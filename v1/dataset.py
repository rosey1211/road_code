            ...
            for r, col in enumerate(self._loc_cols):
                ...
                if field:
                    for val in field.split(";"):
                        ...
                        b = fraction_to_bucket(scaled_loc)
...
def fraction_to_bucket(frac: float) -> int:
    """Convert a [0.0, 1.0] width fraction to a bucket index."""
    config._check()
    return min(int(frac * config.N_BUCKETS), config.N_BUCKETS - 1)
...
def bucket_center_fraction(bucket_idx: int) -> float:
    """Return the fractional centre of a bucket."""
    config._check()
    return (bucket_idx + 0.5) / config.N_BUCKETS
