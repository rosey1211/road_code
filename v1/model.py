...
class RoadCNN(nn.Module):
    ...
    def _row_features(self, feat_map, row_frac_in_crop):
        """
        ...
        and resize horizontally to n_buckets.

        Returns [B, 512, N_BUCKETS].
        """
        B, C, H, W = feat_map.shape
        # Target row in feature map coordinates
        row_idx = max(0, min(H - 1, int(row_frac_in_crop * H)))
        # Take a ±1 row window so the strip is at least 1 row even at edges
        y0 = max(0,     row_idx - 1)
        y1 = min(H,     row_idx + 2)
        strip  = feat_map[:, :, y0:y1, :]                        # [B, C, k, W]
        pooled = F.adaptive_avg_pool2d(strip, (1, self.n_buckets))  # [B, C, 1, N]
        return pooled.squeeze(2)                                   # [B, C, N]
    ...
    def forward(self, x):
        ...
        r0 = self.head_row0(self._row_features(feat, self.row_fracs_in_crop[0])).squeeze(1)
        r1 = self.head_row1(self._row_features(feat, self.row_fracs_in_crop[1])).squeeze(1)
        r2 = self.head_row2(self._row_features(feat, self.row_fracs_in_crop[2])).squeeze(1)
